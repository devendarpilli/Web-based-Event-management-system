	...
	var methods = {
		init: function(options) {
			...
			return this.each(function() {
				if (o.items == 0 || o.items < o.itemsOnPage)
					return;
		...
		_appendItem: function(panel, pageIndex, opts) {
			...
			if (pageIndex == o.currentPage) {
				...
				$link = $('<a style="cursor:pointer;">' + (options.text) + '</a>');
				$link.click(function(){
					methods._selectPage.call(panel, pageIndex, $link);
					return false;
				});
			}
			if (options.classes) {
		...
		_selectPage: function(pageIndex, $item) {
			var o = $(this).data('pagination');
			o.currentPage = pageIndex;
			if (o.selectOnClick) {
				o.onClick(pageIndex + 1, $item);
				methods._draw.call(this);
			} else {
				o.onClick(pageIndex + 1, $item);
			}
